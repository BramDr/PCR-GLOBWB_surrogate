...
for routing_type in routing_types:
    ...
    save_routing_dir = pl.Path("{}/{}".format(save_dir, routing_type))
    ...
    startup_database_file = pl.Path("{}/startup_database.db".format(save_routing_dir))
    database_file = pl.Path("{}/database.db".format(out_routing_dir))
    if not database_file.exists():
        database_file.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy(src=startup_database_file, dst=database_file)
    ...
    sampler = samplers.TPESampler(seed=seed + n_trials)
    ...
    study.optimize(optimize_trial, n_trials=10)
