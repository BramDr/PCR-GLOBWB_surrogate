...
for submask in submasks:
    ...
    for sequence in sequences:
        ...
        if inflow_file.exists() and inflow_meta_file.exists():
            inflow = np.load(inflow_file)
            inflow_meta = load_meta(file = inflow_meta_file)
        ...
        for subset_file in subset_files:
            ...
            upstream_sel = np.isin(ldd["downstream"], subset_samples)
            upstream_samples = ldd.index[upstream_sel]
            
            sequence_samples.append(subset_samples)
            sequence_upstream_samples.append(upstream_samples)
        sequence_samples = np.concatenate(sequence_samples)
        sequence_upstream_samples = np.concatenate(sequence_upstream_samples)
        
        sequence_inflow = None
        sequence_inflow_meta = None
        transfer_inflow = None
        transfer_inflow_meta = None
        if inflow is not None and inflow_meta is not None:
            sequence_sel = np.isin(inflow_meta["samples"], sequence_upstream_samples)
            
            sequence_inflow = inflow[sequence_sel, ...]
            sequence_inflow_meta = copy.deepcopy(inflow_meta)
            sequence_inflow_meta["samples"] = sequence_inflow_meta["samples"][sequence_sel]
            
            transfer_sel = np.isin(inflow_meta["samples"], ldd.index)
            transfer_sel = np.logical_and(~sequence_sel, transfer_sel)
            
            transfer_inflow = inflow[transfer_sel, ...]
            transfer_inflow_meta = copy.deepcopy(inflow_meta)
            transfer_inflow_meta["samples"] = transfer_inflow_meta["samples"][transfer_sel]
        ...
        for subset_file in subset_files:
            ...
            if predicted_file.exists() and predicted_meta_file.exists():
                ...
                subset_inflow = None
                if sequence_inflow is not None and sequence_inflow_meta is not None:
                    subset_inflow = subset_upstream_array(array = sequence_inflow,
                                                          array_samples = sequence_inflow_meta["samples"],
        ...
        if transfer_inflow is not None and transfer_inflow_meta is not None:
            discharges.insert(0, transfer_inflow)
            discharge_metas.insert(0, transfer_inflow_meta)
