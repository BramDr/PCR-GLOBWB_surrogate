...
dir_out = pl.Path("./saves/global_30min/prepare/sequential")
datasets = ["input"]
...
for dataset in datasets:
    ...
    df_files = [file for file in prepare_dir.rglob("*{}_*_ts.csv".format(dataset))]
    plot_dfs = []
    ...
    df_files = [file for file in prepare_dir.rglob("*{}_*_cdf.csv".format(dataset))]
    plot_dfs = []
    for df_file in df_files:
        plot_df = pd.read_csv(df_file, index_col=0)
        plot_dfs.append(plot_df)
    ...
    df_files = [file for file in prepare_dir.rglob("*{}_*_sp.csv".format(dataset))]
    plot_dfs = []
    for df_file in df_files:
        plot_df = pd.read_csv(df_file, index_col=0)
        plot_dfs.append(plot_df)
