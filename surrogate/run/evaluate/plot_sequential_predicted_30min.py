...
pred_df_files = [file for file in predict_dir.rglob("predicted_*_ts.csv")]
pred_plot_dfs = []
...
plot_out = pl.Path("{}/predicted_cdf.pdf".format(dir_out))
plot_out.parent.mkdir(parents=True, exist_ok=True)
plots = plot_cdf_seperate(input_df=plot_df)
pn.save_as_pdf_pages(plots = plots, filename= plot_out)

pred_df_files = [file for file in predict_dir.rglob("predicted_*_sp.csv")]
pred_plot_dfs = []
for pred_df_file in pred_df_files:
    pred_plot_df = pd.read_csv(pred_df_file, index_col=0)
    pred_plot_dfs.append(pred_plot_df)
...
plot_out = pl.Path("{}/predicted_sp.pdf".format(dir_out))
plot_out.parent.mkdir(parents=True, exist_ok=True)
