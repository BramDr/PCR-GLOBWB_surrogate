...
for resolution in resolutions:
    ...
    routing_types = [dir.stem for dir in train_resolution_dir.iterdir() if dir.is_dir()]
    ...
    for routing_type in routing_types:
        ...
        train_routing_dir = pl.Path("{}/{}".format(train_resolution_dir, routing_type))
        out_routing_dir = pl.Path("{}/{}".format(out_mresolution_dir, routing_type))

        setup_trainset_dir = pl.Path("{}/{}".format(setup_routing_dir, trainset))
        
        subsamples = [dir.stem for dir in train_routing_dir.iterdir() if dir.is_dir()]
        subsample = subsamples[0]
        train_subsample_dir = pl.Path("{}/{}".format(train_routing_dir, subsample))
